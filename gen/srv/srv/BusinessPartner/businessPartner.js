    ...
    #Predict_Income_STMNT() {
        for( var rule of this.#Prediction_Rules) {
            ...
            for(var data of this.#incomestmt_qtr) {
                ...
                IncmStmnt_P['totalRevenue'] = ((data.totalRevenue ) + (data.totalRevenue * rule.Sales_Growth ) / 100 ).toFixed(2) ;
                IncmStmnt_P['costOfGoodSold'] = ((data.costOfRevenue * rule.COGS_Margin)/100 ).toFixed(2);
                IncmStmnt_P['SG_A_Expense'] = (( data.sellingGeneralAdministrative * rule.Cost_Inflation )/100).toFixed(2);
                IncmStmnt_P['R_D_Expense'] = (data.researchDevelopment).toFixed(2);
                IncmStmnt_P['otherOperatingIncomeExpense'] = (( data.totalOperatingExpenses * rule.Cost_Inflation)/100).toFixed(2);
                ...
                parseFloat(IncmStmnt_P['R_D_Expense']) +  parseFloat(IncmStmnt_P['otherOperatingIncomeExpense']) ));
                IncmStmnt_P['interestincome'] = ((data.incomeBeforeTax* rule.Cost_Inflation)/100).toFixed(2);
                IncmStmnt_P['interestExpense'] = (( data.interestExpense * rule.Cost_Inflation)/100).toFixed(2);
                IncmStmnt_P['OtherIncomeExpense'] = (( data.totalOtherIncomeExpenseNet * rule.Cost_Inflation)/100).toFixed(2);
                ...
                IncmStmnt_P['netIncomeAfterTax'] = ((  parseFloat(IncmStmnt_P['operatingProfit']) +  parseFloat(IncmStmnt_P['interestincome']) -  parseFloat(IncmStmnt_P['interestincome']) +  parseFloat(IncmStmnt_P['interestExpense']) -  parseFloat(IncmStmnt_P['incomeTax']) ));
                ...
                for (let i = 1; i < 5; i++) {
                    ...
                    IncmStmnt_P['R_D_Expense'] = (parseFloat(L_IncmStmnt_P['R_D_Expense']));
